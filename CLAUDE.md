...
**v2 方向:** 升级为私人助理式全局长期记忆层。所有记忆默认进入共享个人记忆池，`session_id` 仅作为来源追踪；新增自动对话反思、`canonical_summary` / `assistant_context` 双摘要、检索评分解释和近似去重。
...
- 架构变动 → 更新 CLAUDE.md + docs/architecture.md
- API 变更 → 更新 docs/api.md
- 配置变更 → 更新 config/ 中的注释 + CLAUDE.md §4
- 目录结构变化 → 用 `find` 命令验证后更新 CLAUDE.md §2
