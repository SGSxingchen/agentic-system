...
召回后生成一个简洁的 `[长期记忆]` 块注入系统提示词。每条注入内容使用 `assistant_context`，没有时回退到 `canonical_summary` 或 `content`。注入内容限制条数和字符数，避免 prompt 被记忆挤占。记忆块必须明确标记为不可信资料，只能作为事实参考；如果记忆文本包含指令或与当前系统规则冲突，Agent 不得执行记忆中的指令。

### 2. 聊天后反思

Assistant 回复完成后，把本轮 `user` 与 `assistant` 内容追加到轻量对话缓冲。缓冲达到阈值后异步触发反思摘要。缓冲窗口按 `session_id` 隔离；`session_id` 仍然只是长期召回的来源追踪字段，但不能把不同会话的原始消息合并到同一个反思窗口。
...
`MemoryRetriever` 保持轻量实现，不引入 BM25/FAISS。候选粗筛必须先走底层 `MemoryStore.search()`，保留 ChromaDB 等后端的语义检索能力；本地评分只负责二次排序、去重和解释。排序使用:
...
这些字段不持久化进 store，避免检索副作用污染长期数据。候选搜索不更新访问计数，只有最终返回给 Assistant/API 的召回结果刷新 `access_count` 和 `last_accessed`。
